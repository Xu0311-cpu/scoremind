...
def test_api_analyze_musicxml_returns_structured_json() -> None:
    ...
    assert payload["analysis_version"] == "3.3.0"
    ...
    assert "MVP 3.3 detects chords only from simultaneous pitch sets at identical offsets." in payload["warnings"]
...
def test_explain_analysis_returns_template_explanation_for_valid_analysis_json() -> None:
    ...
    assert payload["analysis_version"] == "3.3.0"
    assert payload["explanation_version"] == "3.3.0"
    ...
    assert "No LLM is called in MVP 3.3." in payload["warnings"]
...
def test_api_invalid_file_extension_returns_400() -> None:
    ...
    assert response.status_code == 400
    assert response.json()["detail"] == "Only .musicxml and .xml files are supported in MVP 3.3."
