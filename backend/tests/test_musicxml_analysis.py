...
def test_api_analyze_musicxml_returns_structured_json() -> None:
    ...
    assert payload["analysis_version"] == "2.6.1"
    ...
    assert "MVP 2.6.1 detects chords only from simultaneous pitch sets at identical offsets." in payload["warnings"]
    ...
    assert (
        "MVP 2.6.1 note-level analysis prefers same-offset harmony, then may use carried previous chord context within the same measure."
    ) in payload["warnings"]
...
def test_explain_analysis_returns_template_explanation_for_valid_analysis_json() -> None:
    ...
    assert payload["analysis_version"] == "2.6.1"
    assert payload["explanation_version"] == "2.6.1"
    ...
    assert "No LLM is called in MVP 2.6.1." in payload["warnings"]
...
def test_api_invalid_file_extension_returns_400() -> None:
    ...
    assert response.status_code == 400
    assert response.json()["detail"] == "Only .musicxml and .xml files are supported in MVP 2.6.1."
