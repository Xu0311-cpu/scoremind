...
def test_api_analyze_musicxml_returns_structured_json() -> None:
    ...
    assert payload["analysis_version"] == "3.7.0"
    ...
    assert "MVP 3.7 detects chords only from simultaneous pitch sets at identical offsets." in payload["warnings"]
    ...
    assert (
        "MVP 3.7 note-level analysis prefers same-offset harmony, then may use carried previous chord context within the same measure."
    ) in payload["warnings"]
...
def test_explain_analysis_returns_template_explanation_for_valid_analysis_json() -> None:
    ...
    assert payload["analysis_version"] == "3.7.0"
    assert payload["explanation_version"] == "3.7.0"
    ...
    assert "No LLM is called in MVP 3.7." in payload["warnings"]
...
def test_api_invalid_file_extension_returns_400() -> None:
    ...
    assert response.status_code == 400
    assert response.json()["detail"] == "Only .musicxml and .xml files are supported in MVP 3.7."
...
def test_harmonic_context_exists_on_measures() -> None:
    ...
    for measure in response.json()["measures"]:
        ...
        assert ctx["confidence"] in ("supported", "partial", "low")


def test_harmonic_context_supported_for_c_major_progression() -> None:
    client = TestClient(app)
...
def test_harmonic_context_low_for_no_detected_chord() -> None:
    client = TestClient(app)

    with (FIXTURE_DIR / "chord_quality_matrix.musicxml").open("rb") as handle:
        response = client.post(
            "/api/v1/analyze/musicxml",
            files={"file": ("chord_quality_matrix.musicxml", handle, "application/vnd.recordare.musicxml+xml")},
        )

    assert response.status_code == 200
    measures = response.json()["measures"]

    empty_measures = [m for m in measures if len(m["detected_chords"]) == 0]
    assert len(empty_measures) > 0
    for measure in empty_measures:
        ctx = measure["harmonic_context"]
