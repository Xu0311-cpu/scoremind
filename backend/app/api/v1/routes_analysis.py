...
MVP_WARNINGS = [
    "MVP 3.1 detects chords only from simultaneous pitch sets at identical offsets.",
    ...
    "Passing tone and neighbor tone detection are not performed in MVP 3.1.",
]
...
async def analyze_musicxml(file: UploadFile) -> MusicXMLAnalysisResponse:
    if not _looks_like_musicxml(file.filename):
        raise HTTPException(
            ...
            detail="Only .musicxml and .xml files are supported in MVP 3.1.",
        )
