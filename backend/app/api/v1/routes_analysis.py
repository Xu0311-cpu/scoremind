...
MVP_WARNINGS = [
    "MVP 2.6.1 detects chords only from simultaneous pitch sets at identical offsets.",
    ...
    "Passing tone and neighbor tone detection are not performed in MVP 2.6.1.",
]

ANALYSIS_VERSION = "2.6.1"
ANALYSIS_SCOPE = [
    "musicxml_input_only",
...
async def analyze_musicxml(file: UploadFile) -> MusicXMLAnalysisResponse:
    if not _looks_like_musicxml(file.filename):
        raise HTTPException(
            ...
            detail="Only .musicxml and .xml files are supported in MVP 2.6.1.",
        )
