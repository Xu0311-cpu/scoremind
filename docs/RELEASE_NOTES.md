...
详见 `docs/VALIDATION.md`。本地后端 90 项、前端 11 项单测和构建通过；浏览器已覆盖真实 MusicXML、新增/编辑、重载、换谱隔离、错谱拒绝、报告隔离及 390px/1280px 无横向溢出。内置浏览器未上报 blob 下载事件，后续文件选择出现超时；内联删除/导入覆盖确认、下载落盘和真实触屏硬件手势仍需独立复审补验。本地通过不代表远端 CI 已运行。
